...
export default function Login() {
  ...
  // Handle incoming OAuth redirect query params (?token=... or ?oauth_error=...)
  useEffect(() => {
    const token = searchParams.get("token");
    const oauthError = searchParams.get("oauth_error");

    if (token) {
      setOauthLoadingProvider("authenticating");
      (async () => {
        try {
          ...
          setErrorMessage(error.message || "Failed to complete social authentication.");
  ...
  }, [searchParams, loginWithToken, navigate, setSearchParams, showToast]);
  ...
  function handleSocialLogin(provider) {
    ...
    const endpoint = provider.toLowerCase() === "google" ? "/api/auth/google" : "/api/auth/github";
